...
First, we introduce the \textbf{Audience-Brand Alignment Index (ABAI)}, a
...
(ABAI\,=\,0.674). Second, we build a bot-detection classifier combining
...
is, to our knowledge, the first attempt to do that in closed form.
...
algorithm shift. The burstiness inversion we document here is, to our
...
For the bot-detection experiment we construct a corpus of 1{,}000 engagement
...
For the temporal stability experiment we generate 1{,}000 campaign records
...
From ABAI we derive two quantities. The adjusted campaign value scales
...
with 5-fold stratified cross-validation. We also evaluate each feature in
isolation to identify which signals are carrying the weight.

\subsection{ROI Temporal Stability}

For each feature family $F \in \{\text{behavioral, semantic}\}$ we train
...
actual ROI on the held-out 20\%. We repeat the evaluation on the post-2023
